export default {
  root: ({props, state}) => ({
    class: [
      ...
      {'bg-grays-light-100 hover:bg-grays-light-200': props.variant === 'filled'},

      // Border
      {'outline-0': props.variant === 'filled'},
      ...
      {'outline-none outline-offset-0 z-10 ring-1 ring-primary-500 dark:ring-primary-400': state.focused && props.variant !== 'filled'},
      {'bg-grays-light-200': state.focused && props.variant === 'filled'},

      // Misc
      'cursor-pointer',
      'select-none',
      {'bg-grays-light-50 dark:bg-surface-700 select-none pointer-events-none cursor-default': props.disabled}
    ]
  }),
  label: ({props}) => ({
    class: [
      ...
      {'text-grays-light-300': props.disabled}
    ]
  }),
  ...
  item: ({context}) => ({
    class: [
      ...
      // Disabled
      {'opacity-60 pointer-events-none cursor-default bg-grays-light-100 text-grays-light-400': context.disabled}
    ]
  }),
  content: {
    class: [
